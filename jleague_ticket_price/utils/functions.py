...
def ticket_price_scraping():
    ...
    try:
            ...
            try:
                for element in game_list.find_all("li"):
                    ...
                    try:
                        ...
                        match_title = (
                            element.find(class_="vs-box-vs-place").find("p").text
                        )

                    # スタジアム名を取得
                    try:
                        stadium_name = (
                            element.find(class_="vs-box-place").find("span").text
                        )
                    except AttributeError as e:
                        stadium_name = (
                    ...
                    for ticket_element in ticket_list.find_all(
                        ...
                        if not SeatCategory.objects.filter(
                            ...
                            print(
                                f"未登録の座席カテゴリ名：{seat_category} を登録しました"
                            )
                        else:
        ...
        logger.error("エラー発生 in %s: %s", ticket_price_scraping.__name__, e)
